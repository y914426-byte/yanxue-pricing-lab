...
export function SchemeCostingPanel({ schemeId }: { schemeId: string }) {
  ...
  useEffect(() => {
    ...
    void request<CostingResponse>('/api/scheme-costing?schemeId=' + encodeURIComponent(schemeId), { signal: controller.signal })
      .then((data) => {
  ...
  return (
    <section className="panel scheme-costing" aria-labelledby="scheme-costing-heading">
      ...
      {costing && <>
        ...
        <div className="scheme-costing-footer"><div>{costing.knownPerPerson === null ? <span>已知人均成本：待确认</span> : <span>已知人均成本：¥{money(costing.knownPerPerson)} / 人</span>}<small>{costing.unresolvedCount > 0 ? '不包含尚未确定的成本项目' : '当前所有未移除项目均已确定数量和价格'}</small></div>{costing.allRequiredResolved && <strong>当前可计算总成本：¥{money(costing.knownCostTotal)}</strong>}<Button disabled={busy || !costing.items.some((item) => !item.removed && item.total !== null)} onClick={bringToPricing}>带入研学定价台</Button><Button disabled={busy} onClick={() => void confirmLearning()}>确认并学习</Button>{latestBatch && !latestBatch.revokedAt && <Button variant="ghost" disabled={busy} onClick={() => void revokeLearning()}>撤销本次学习</Button>}</div>
      </>}
