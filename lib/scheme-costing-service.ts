...
async function parseBody(request: Request) {
  ...
  if (
    ...
    !PRICE_SOURCES.includes(priceSource as PriceSource)
  )
  ...
  if (body.groupType !== undefined) {
    if (
      ...
      body.groupType === 'unknown' ||
      !GROUP_TYPES.includes(body.groupType as SchemeAnalysis['summary']['groupType'])
    )
  ...
  if (body.decisions !== undefined) {
    if (!body.decisions || typeof body.decisions !== 'object' || Array.isArray(body.decisions))
      throw new Error('价格选择格式无效');
    for (const [key, selected] of Object.entries(body.decisions as Record<string, unknown>)) {
      if (!/^\d{1,3}$/.test(key) || (selected !== null && typeof selected !== 'string'))
        throw new Error('价格选择格式无效');
      if (typeof selected === 'string' && (!selected || selected.length > 100))
        throw new Error('价格选择格式无效');
      decisions[key] = selected as string | null;
    }
  }
  return { schemeId, priceSource: priceSource as PriceSource, groupType, decisions };
}
...
async function loadLatestAnalysis(db: D1Database, schemeId: string, owner: string) {
  return await db
    .prepare(
      'SELECT id, analysis_json, created_at FROM scheme_analyses WHERE scheme_document_id = ? AND owner_id = ? ORDER BY created_at DESC, id DESC LIMIT 1',
    )
    .bind(schemeId, owner)
    .first<AnalysisRow>();
}
...
async function loadAvailableItems(db: D1Database, owner: string, source: PriceSource) {
  ...
  const { results } = await db
    .prepare(
      `SELECT pi.*, pc.name AS catalog_name, pc.version AS catalog_version, pc.source AS catalog_source
       FROM price_items pi
       JOIN price_catalogs pc ON pc.id = pi.catalog_id
       WHERE pc.is_active = 1 ${sourceClause}
       ORDER BY pc.updated_at DESC, pi.sort_order, pi.id
       LIMIT 3000`,
    )
    .bind(...bindings)
...
export async function schemeCostingRequest(
  request: Request,
  owner: string | null,
  getDb: () => D1Database,
) {
  ...
  if (request.method === 'POST' && !sameOrigin(request))
    return reply({ error: '请求来源无效，请刷新后重试' }, 403);

  let schemeId: string;
  let source: PriceSource = 'personal_first';
  let groupType:
    | Exclude<SchemeAnalysis['summary']['groupType'], 'unknown'>
    | undefined;
  ...
  if (request.method === 'GET') {
    try {
      ...
      if (requested) {
        if (!PRICE_SOURCES.includes(requested as PriceSource))
          throw new Error('价格来源无效');
        source = requested as PriceSource;
      }
    } catch (error) {
    ...
    try {
      ({ schemeId, priceSource: source, groupType, decisions } = await parseBody(request));
    } catch (error) {
      return reply({ error: error instanceof Error ? error.message : '请求格式无效' }, 400);
    }
  }
  ...
  try {
    ...
    if (!analysisRow)
      return reply(
        { error: '请先完成智能成本分析', code: 'ANALYSIS_REQUIRED' },
        409,
      );
    ...
    if (search !== null) {
      ...
      return reply({
        ...
        items: items
          .filter((item) =>
            [item.name, item.category, item.projectName]
              .join(' ')
              .toLocaleLowerCase('zh-CN')
              .includes(query),
          )
          .slice(0, 100)
          .map(serializeSearchItem),
      });
    }
    ...
    if (request.method === 'GET') {
      const row = await db
        .prepare(
          'SELECT * FROM scheme_cost_estimates WHERE scheme_document_id = ? AND owner_id = ? ORDER BY updated_at DESC, id DESC LIMIT 1',
        )
        .bind(schemeId, owner)
        .first<CostingRow>();
      return row
        ? reply(parseStoredResult(row))
        : reply({
            costing: null,
            schemeId,
            schemeAnalysisId: analysisRow.id,
            priceSource: null,
          });
    }

    const items = await loadAvailableItems(db, owner, source);
    const result = costScheme(
      analysis,
      analysisRow.id,
      items,
      source,
    ...
    await db
      .prepare(
        `INSERT INTO scheme_cost_estimates
