...
export async function estimateRequest(request:Request,owner:string|null,db:()=>D1Database){
 ...
 return reply({items:results.slice(0,20).map(row=>{const {plan}=parseEstimate({id:row.id,title:row.title,plan:JSON.parse(row.plan_json)});const r=calculate(plan);return {id:row.id,title:row.title,createdAt:row.created_at,paying:plan.paying,price:plan.price,actual:r.actual,profit:r.profit};}),hasMore:results.length>20});
